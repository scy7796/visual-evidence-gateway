...
在 2026-08-06 发布验收的 Windows 11 构建环境（CPython 3.12.10、24 个可见 CPU）中，使用一张 1920×1080、108,554 字节 PNG，排除网络和模型推理后，实测：

| 路径 | 次数 | 中位数 | P95 | 后端调用 |
|---|---:|---:|---:|---:|
| 未命中缓存的完整本地流水线 | 80 | 92.4 ms | 146.6 ms | 每次 1 次模拟后端 |
...
同一构建环境中的完整源码回归：197 项测试通过、9 项平台性跳过（如 POSIX 安装器集成测试在 CI 的 ubuntu 上运行），进程墙钟约 7 秒。该数字可以说明开发反馈环较快，但不能替代真实宿主和真实 Luna 验收。
